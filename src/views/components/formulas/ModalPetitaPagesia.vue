<template>
  ...
  <ion-content class="ion-padding">
    <h1 class="modal-title">{{ $t('indicators.formulas.petita_pagesia.title') }}</h1>
    <p>{{ $t('indicators.formulas.petita_pagesia.message') }}</p>

    <p class="alert-info" v-show="required_num_workers_missmatch">
      <ion-icon size="large" :icon="infoBinIcon()"></ion-icon>
        <span>
        {{
          $t('indicators.formulas.petita_pagesia.info_num_workers', {
            total: this.required_num_workers,
          })
        }}
        </span>
    </p>

    <div v-show="centres_treball.length>0">
      <div v-for="(centreTreball, key) in centres_treball" :key="key" class="centre-treball">
        <div style="display: flex;align-items: center;gap: 1rem;">
          ...
          <ion-select
              ...
              :label="$t('indicators.formulas.petita_pagesia.location_and_name_label')"
          ...
          <ion-button type="button" size="small" fill="clear" color="danger"
            ...
            {{ $t('indicators.formulas.petita_pagesia.delete') }}
          </ion-button>
        </div>
        ...
        <div>
          <div v-for="(workerOption, workersOptionsKey) in centreTreball.workersOptions" :key="workersOptionsKey"
            ...
            <div style=" flex: 0 0 260px;margin: 1rem 0;">
              <ion-select
                  class="input-custom" fill="solid" label-placement="stacked"
                  :label="$t('indicators.formulas.petita_pagesia.option')"
            ...
            <div style=" flex: 0 0 180px;">
              <ion-input
                  class="input-custom" fill="solid" label-placement="stacked"
                  :label="$t('indicators.formulas.petita_pagesia.num_workers')"
        ...
        <ion-button type="button" fill="outline" @click="addWorkersOption(centreTreball)"
          ...
          {{ $t('indicators.formulas.petita_pagesia.add_option_btn') }}
        </ion-button>
      </div>
    </div>
    ...
    <div style="text-align: right;">
      <ion-button type="button" fill="outline" @click="addCentreTreball" style="margin-bottom: 1.5rem;" size="small">
        <ion-icon slot="start" :icon="addIcon()"></ion-icon>
        {{ $t('indicators.formulas.petita_pagesia.add_centre_btn') }}
      </ion-button>
    </div>

    <h5>{{ $t('indicators.formulas.petita_pagesia.result') }}</h5>
...
export default defineComponent({
  ...
  methods: {
    ...
    update() {
      if (this.checkIfRequiredNumWorkersMismatch()) {
        SharedMethods.showErrorToast(
            this.t('indicators.formulas.petita_pagesia.num_workers_missmatch')
        );
        return;
      }
    ...
    checkIfRequiredNumWorkersMismatch() {
      ...
      if (this.required_num_workers != totalWorkers) {
        return true;
      }
      return false;
    }
